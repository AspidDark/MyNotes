...
function FileUpload() {
    ...
    const handleFileUpload = async (element: HTMLInputElement) => {
        ...
        const validFileType = await validateFileType(FileUploadService.getFileExtension(file[0].name))
        ...
        const fileUploadService = new FileUploadService(file[0])
        const fileUploadResponse = await fileUploadService.uploadFile()
